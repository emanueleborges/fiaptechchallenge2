...
# Política para Lambda Scraper
resource "aws_iam_role_policy" "lambda_scraper_policy" {
  name = "${var.project_name}-lambda-scraper-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "${aws_s3_bucket.bovespa_data.arn}/*"
      },
      {
        Effect = "Allow"
        Action = [
...
# Política padrão do Glue
resource "aws_iam_role_policy_attachment" "glue_service_role" {
  role       = aws_iam_role.glue_job_role.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSGlueServiceRole"
}

# Política customizada para Glue Job
...
# Role para EventBridge (agendamento)
    ...
    Statement = [
      {
        ...
        Resource = aws_lambda_function.bovespa_scraper.arn
      }
    ]
  })
}
